...
class Libero_Spatial_Attack(Libero_Tabletop_Manipulation):
    ...
    def _reset_milp(self):
        ...
        context.cplex_parameters.dettimelimit = self._repair_config['time_limit']
        ...
        for this_obj, other_obj in combinations(
            ...
            if (
                ...
                if this_z >= other_z:
                    ...
                    min_clearance = abs(this_obj.bottom_offset[-1]) + abs(other_obj.top_offset[-1]) 
                else:
                    # other_obj is on top
                    clearance = other_z - this_z
                    min_clearance = abs(this_obj.top_offset[-1]) + abs(other_obj.bottom_offset[-1]) 
    ...
    def _milp_build_basic_problem(self):
        ...
        for this_obj, other_obj in combinations(
            ...
            if (
                (this_movable or other_movable) 
                and 
                ((clearance < min_clearance - 1e-6) or 
                this_obj.name == "plate_1" or 
                other_obj.name == "plate_1")
            ):
                threshold = this_obj.horizontal_radius + \
                    other_obj.horizontal_radius + 0.01
...
class Task_0(Libero_Spatial_Attack):
    ...
    def _check_valid_env_task(self):
        xl, yl, xh, yh = self.parsed_problem["regions"]["main_table_table_center"]["ranges"][0]
...
class Task_2(Libero_Spatial_Attack):
    ...
    def _milp_build_task_problem(self):
        bowl_1_x_var = self._mdl.get_var_by_name("akita_black_bowl_1_x")
        bowl_1_y_var = self._mdl.get_var_by_name("akita_black_bowl_1_y")
        ramekin_x_var = self._mdl.get_var_by_name("glazed_rim_porcelain_ramekin_1_x")
        ramekin_y_var = self._mdl.get_var_by_name("glazed_rim_porcelain_ramekin_1_y")
...
class Task_7(Libero_Spatial_Attack):
    ...
    def _check_valid_env_task(self):
        ...
        if not (
            abs(bowl_1_x - midpoint_x) <= self.tolerance*abs(ramekin_x - plate_x) 
            and 
    ...
    def _milp_build_task_problem(self):
        ...
        self._mdl.add_constraint(
            ...
            <= self.tolerance*self._mdl.abs(ramekin_x_var - plate_x_var) - 1e-6
        )
        bowl_1_y_var = self._mdl.get_var_by_name("akita_black_bowl_1_y")
        self._mdl.add_constraint(
            self._mdl.abs(bowl_1_y_var - midpoint_y_var) 
            <= self.tolerance*self._mdl.abs(ramekin_y_var - plate_y_var) - 1e-6
        )
        bowl_2_x_var = self._mdl.get_var_by_name("akita_black_bowl_2_x")
        self._mdl.add_constraint(
...
class Task_8(Libero_Spatial_Attack):
    ...
    def _check_valid_env_basic(self):
        ...
        for this_obj, other_obj in combinations(
            ...
            # wooden_cabinet_1
            if (
                this_obj.name in ['akita_black_bowl_1', 'wooden_cabinet_1'] and 
                other_obj.name in ['akita_black_bowl_1', 'wooden_cabinet_1']
            ):
                continue
